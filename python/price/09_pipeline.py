...
此程式每次啟動只執行一輪；週期排程交由 Airflow 管理。
"""
...
REFERENCE_WORKBOOK_RELATIVE_PATH = Path("data/reference/food_nutrition_2025.xlsx")
...
def resolve_reference_workbook(directory: Path) -> Path:
    ...
    # 本機執行時：price -> python -> 專案根目錄 -> data/reference。
    project_dir = directory.parent.parent
    candidates.append(project_dir / REFERENCE_WORKBOOK_RELATIVE_PATH)
